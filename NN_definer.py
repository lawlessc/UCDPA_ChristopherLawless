import keras
import pandas as pd
from keras.layers import Dense , Flatten,Dropout,RNN, SimpleRNN,GRU,LSTM ,Reshape ,Conv1D,Conv2D, MaxPooling1D, MaxPooling2D
...
class NN_definer:
    ...
    def create_model(self, first_layer=30,hidden_layers=1, layer_widths=10, optimizer="sgd", winit="glorot_uniform",cnn_window_size=30, dropout=0.01, decay =0.01, ):
        ...
        this is mainly for use with GridSearchCV but can be used discretely if you just want a model'''
        model = Sequential()

        #Input Layer
        model.add(Reshape((12288, 1), input_shape=(12288,)))
        model.add(Conv1D(first_layer,cnn_window_size, activation='relu', input_shape=(12288, 1),kernel_initializer=winit))
        ...
        for x in range(hidden_layers):
            ...
            model.add(Conv1D(cnn_window_size, 10, activation='relu'))
            #model.add(Dense(int(layer_widths),activation="relu", kernel_initializer=winit))


        model.add(Flatten())# This flattening layer is needed to avoid breaking categorical cross entropy. otherwise output is multidimensional
    ...
    def trainer_loop(self, predictors, target,model):
        '''This is just a loop for training a model multiple times, it takes predictors,targets and the model object as input'''
